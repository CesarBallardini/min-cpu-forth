...
kernel that runs on top of it; **Phases 0-2 and 4 (the assembler, the ITC threading core, the core
primitive word set, and the I/O + dictionary-search words) are built** -- a colon definition runs
through genuine `NEXT`/`DOCOL`/`EXIT`, and `KEY`/`EMIT`/`WORD`/`FIND`/`NUMBER` are in place -- while
Phase 3 (branching/loops) is skipped for now and Phases 5+ are not. See `CLAUDE.md` for how the
pieces fit together.
...
src/min_cpu_forth/         # hexagon; dependency arrows point inward (enforced by .importlinter)
  ...
  ports.py                 # port Protocols: memory, stack, registers, char I/O, dictionary, sys-vars, assembler
  ...
  adapters/                # concrete port impls (memory, stack, registers, char I/O, source, dictionary, sys-vars)
...
docs/
  ...
  03-assembler-plan.md     # phased plan for an ITC Forth kernel (Phases 0-2 + 4 built; Phase 3, 5+ pending)
